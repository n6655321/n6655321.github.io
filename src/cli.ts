...
function parseArgs(argv: string[]): ParsedArgs | null {
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case "-s":
      case "--serve": {
        // The port is optional, so only consume the next token if it is a number.
        const peek = args[i + 1];
      ...
      default:
        ...
        if (vault !== null) throw new Error(`Unexpected argument: ${arg}`);
        vault = arg;
    }
  }

  if (vault === null) throw new Error("Missing <vault> argument.");
  ...
  return {
    ...
    // Normalise: no trailing slash, leading slash if non-empty.
...
async function main(): Promise<void> {
  ...
  /** Print a build's summary and any authoring warnings. */
  function report(result: BuildResult, elapsed: number): void {
    ...
    const rooms = result.rooms
      ...
      : "";
    console.log(
      `tektite: ${result.notes} notes, ${result.tags} concepts${rooms}, ` +
        `${result.pages} pages in ${elapsed}ms`,
    );

    // Authoring mistakes in a room note are easy to make and invisible in the
    // output, so surface them rather than shipping a room with dead objects.
  ...
  const server =
    options.serve !== null
      ...
      : null;
  if (server) console.log(`         serving at ${server.url}`);
  ...
  const watcher = await watchVault(options, {
    ...
    onError: (error) => {
      // A broken build must not kill the watcher: the next save usually fixes it.
