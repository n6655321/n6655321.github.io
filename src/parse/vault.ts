/** Walks an Obsidian vault and turns its markdown files into `Note`s. */
...
/** Convert a filesystem path to the POSIX form used throughout the index. */
function toPosix(p: string): string {
  return p.split(path.sep).join("/");
}

/** Recursively list vault-relative paths of every `.md` file. */
export async function walkVault(
  ...
  async function walk(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (ignorePaths.some((p) => rel === p || rel.startsWith(`${p}/`))) continue;
...
/** Strip markdown syntax down to readable prose for excerpts. */
...
/** First H1 in the body, if there is one. */
function firstHeading(md: string): string | null {
  const m = /^[ \t]{0,3}#\s+(.+)$/m.exec(md);
  return m ? m[1]!.trim() : null;
}

/** Targets of every `[[wikilink]]`, stripped of aliases and block anchors. */
...
/** Parse one markdown file into a `Note`. */
export async function readNote(root: string, rel: string): Promise<Note> {
  const raw = await fs.readFile(path.join(root, rel), "utf8");
  // `gray-matter` throws on malformed YAML; a bad header shouldn't lose the note.
...
export async function readVault(options: BuildOptions): Promise<Note[]> {
  ...
  if (options.ignoreTags.length === 0) return notes;
