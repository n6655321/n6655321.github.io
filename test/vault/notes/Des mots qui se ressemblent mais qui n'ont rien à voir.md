...
Concrètement, sur le plan extensionnel, cela consiste à simultanément préférer A à B, B à C et C à A (A>B ^ B>C ^ C>A). Étant donné l'*équivalence extensionnelle* de C et A (A <=> C), et en vertu de la *substituabilité des identiques*, préférer B à C revient à préférer B à A ((B>C)<=>(B>A)). Finalement, cela revient à préférer A à B et B à A ((A>B)^(B>A)), ce qui est contradictoire (et de cette contradiction, selon le principe d'explosion ou _ex falso quodlibet_, tout le système s'écroule).
